...
public class SubscriptionController {
	...
	public ResponseEntity<?> createSubscription(@PathVariable String prettyName, @RequestBody User subscriber,
		...
		try {
			SubscrptionResponse res = service.createNewSubscription(prettyName, subscriber, userId);
			if (res != null) {
		...
		} catch (SubscriptionConflictExcecption ex) {
			return ResponseEntity.status(409).body(new ErrorMessage(ex.getMessage()));
		} catch (UserIndicadorNoutFoundException ex) {
			return ResponseEntity.status(409).body(new ErrorMessage(ex.getMessage()));
		}
